...
class AutoDPOModelForCausalLM(PreTrainedModelWrapper):
    ...
    def prediction_step_mcqa(self, batch, tokenizer):
        ...
        for item in batch:
            ...
            # Tokenize the inputs for each choice
            inputs = [tokenizer.encode_plus(question, choice, return_tensors="pt") for choice in choices]

            # Get model outputs for each choice
            choice_scores = []
            for input_data in inputs:
                input_ids = input_data["input_ids"].to(self.pretrained_model.device)
                attention_mask = input_data["attention_mask"].to(self.pretrained_model.device)
                
                with torch.no_grad():
                    outputs = self.pretrained_model(input_ids=input_ids, attention_mask=attention_mask)
                
                # We assume that the model outputs logits for each input, and we take the last token's logits
                logits = outputs.logits
                choice_score = logits[:, -1, :].mean().item()  # Average the logits for the last token
                choice_scores.append(choice_score)

            # Determine the index of the choice with the highest score
            pred_idx = torch.tensor(choice_scores).argmax().item()
            
            # Convert index to corresponding letter (e.g., 0 -> 'A', 1 -> 'B', etc.)
