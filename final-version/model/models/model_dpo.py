...
class AutoDPOModelForCausalLM(PreTrainedModelWrapper):
    ...
    def __init__(self, pretrained_model, **kwargs):
        ...
        if isinstance(pretrained_model, PeftModelForCausalLM):
          pretrained_model.enable_input_require_grads()
          print("Peft model, enabling input_require_grads()")

        if not any(hasattr(self.pretrained_model, attribute) for attribute in self.lm_head_namings):
            raise ValueError("The model does not have a language model head, please use a model that has one.")
        self.use_rag = self.is_rag
        self.use_q =self.is_q
        ###########################################################################################
    ...
    def forward(
        ...
        **kwargs,
        ...
        return Output(logits=outputs.logits)
    ...
    def get_logprobs(self, batch, tokenizer):
        ...
        for prompt, chosen, rejected in zip(batch["prompt"], batch["chosen"], batch["rejected"]):
            ...
            input_ids_rejected = torch.cat([prompt_encodings["input_ids"], rejected_encodings["input_ids"][:, 1:]],
                                           dim=1)

            attention_mask_chosen = torch.cat(
                [prompt_encodings["attention_mask"], chosen_encodings["attention_mask"][:, 1:]], dim=1)
            attention_mask_rejected = torch.cat(
                [prompt_encodings["attention_mask"], rejected_encodings["attention_mask"][:, 1:]], dim=1)

            # Compute logits
            with torch.no_grad():
                outputs_chosen = self.pretrained_model(input_ids=input_ids_chosen, attention_mask=attention_mask_chosen)
                outputs_rejected = self.pretrained_model(input_ids=input_ids_rejected,
            ...
            rejected_token_logps = torch.gather(rejected_logps, 2,
                                                rejected_encodings["input_ids"].unsqueeze(-1)).squeeze(-1)
    ...
    def prediction_step_reward(
        ...
        # ======================================================================
        # Using the DPO reward function as described in the paper
        
        beta= 1 # TO DO: Set the value of beta
    ...
    def prediction_step_mcqa(self, batch, tokenizer, use_quantiz=False):
        """
        ...
        if self.use_rag:
            ...
            ctx_encoder = DPRContextEncoder.from_pretrained(dpr_ctx_encoder).to(device)
            ctx_tokenizer = DPRContextEncoderTokenizer.from_pretrained(dpr_ctx_encoder)
            from datasets import load_dataset, Dataset
            import json
            #with open('documents/retrieved_documents_with_mapping.json', 'r') as f:
                #data = json.load(f)
            #docs= Dataset.from_pandas(pd.DataFrame(data))

            docs = load_dataset("aerdna/retrieved_documents_with_mapping", split='train')
        ...
        for question_text in tqdm(batch["question"], total=len(batch["question"])):
            if self.use_rag:
                question_embedding = question_encoder(
                    **question_tokenizer(question_text, return_tensors="pt", max_length=512, truncation=True).to(
                        device))[0][0].cpu().numpy()
                _, retrieved_examples = docs.get_nearest_examples('embeddings', question_embedding, k=1)
                retrieved = list(set(retrieved_examples['text']))[:1]
                ...
                prompt = f"""Context: {retrieved} {tokenizer.bos_token} [INST] ### Task: You will be asked a question about STEM, more particularly on Computer science, AI, maths or physics related questions. You're a specialist in the field of the question. Your task is to answer the question to the best of your abilities. You must complete the task step by step and precise what is your final answer.
                            ...
                            ### {question_text}.\n ### Options: Use a capital letter.[/INST] \n ### Answer: """
            else:
            ...
            # Generate the output
            ys = self.pretrained_model.generate(
                **tokenizer(prompt, return_tensors='pt', truncation=True).to(device), **GENERATION_KWARGS)

            output_decoded = tokenizer.batch_decode(ys, skip_special_tokens=False)

            if self.use_rag or self.use_q:
                split = output_decoded[0].split("### Answer:")[-1].split("# Answer")[-1][:50]
            else:
                split = output_decoded[0].split("### Answer:")[-1][:50]
            ...
            json_schema = {
                ...
                "required": ["correct_letter"]
            }

            jsonformer = Jsonformer(self.pretrained_model, tokenizer, json_schema, prompt2)
            response = jsonformer()
            ...
            output_dict["preds"].append(generated_answer)
