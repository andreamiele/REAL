...
def main():
    ...
    if use_rag:
        ...
        #docs = docs.map(lambda example: {
            #'embeddings': ctx_encoder(**ctx_tokenizer(example["text"], return_tensors="pt", max_length=512))[0][0].numpy()})
        #print(len(docs))
    ...
    if epfl_data:
        ...
        print('mvujas/stem_mcqa_questions')
    ...
    for i, entry in tqdm(enumerate(test_set), total=nb):
        ...
        if use_rag:
            ...
            _, retrieved_examples = docs.get_nearest_examples('embeddings', question_embedding, k=5)

            retrieved = list(set(retrieved_examples['text']))[:1]#to add somewhere in the prompt
            prompt = f"""
                        Question: {only_question}
                        Use this context to help you answer if needed: {retrieved}
                        Options: {option_lines}

                        Choose the correct letter among the options: {option_lines}
                        Be 100% sure of your answer. I will give you 10B dollars if you are right and 0 dollars if you are wrong.
                        """
        ...
        json_schema = {
            ...
            "required": ["question","options", "correct_letter"]
        }
